...
export function LoginView({ logo }: { logo: string | null }) {
  ...
  return (
    <main className="relative flex min-h-dvh flex-col justify-end overflow-hidden bg-background px-6 pb-16 pt-24 sm:px-12 sm:pb-24">
      ...
      {/* Brand — personal logo from the database, text fallback if unset */}
      <motion.span
        initial={reduce ? { opacity: 0 } : { opacity: 0, y: -12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, ease: [0.23, 1, 0.32, 1] }}
        className="absolute left-6 top-6 z-10 flex items-center font-mono text-sm font-semibold tracking-tight sm:left-12 sm:top-8"
      >
        {logo ? (
          <img
            src={logo}
            alt="Logo"
            className="w-auto object-contain sm:h-5"
          />
        ) : (
          <>
